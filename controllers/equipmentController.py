...
class EquipmentActions:
    ...
    @staticmethod    
    def delete_equipment(errors, equipment_id):
        try:
            conn, cursor = connect_to_base()
    ...
    @staticmethod           
    def edit_equipment(errors, equipment_id, name, count, description):
        try:
            conn, cursor = connect_to_base()
            cursor.execute(
                "UPDATE Equipment SET name = ?, count = ?, description = ? WHERE equipment_id = ?",
                (name, count, description, equipment_id))
            conn.commit()
            close_base(conn)
        except:
            errors.append("Ошибка с связью бд")
    ...
    @staticmethod   
    def add_equipment(hall_id, name, count, description, errors):
        try:
            conn, cursor = connect_to_base()
            cursor.execute(
...
def add_equipment():
    ...
    if not errors:
        errors.append("Оборудование успешно добавлено", "success")
...
def delete():
    ...
    return render_template("hall_equipment.html", errors=errors, hall_id=hall_id, equipment_list=EquipmentActions.get_all_equipment_for_hall(hall_id))



@equipment_bp.route("/equipment_edit", methods=["POST"])
def edit_equipment():
    equipment_id = request.form.get("equipment_id")
    hall_id = request.form.get("hall_id")
    name = request.form.get("name")
